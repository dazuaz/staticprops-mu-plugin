...
class PathsApi extends WP_REST_Controller {
  public function register_routes() {
    ...
    register_rest_route( $namespace, '/' . $base, array(
      array(
        ...
        'permission_callback' => array( $this, 'get_paths_permissions_check' ),
        'args'                => array(),
      ),
    ) );
    register_rest_route( $namespace, '/' . $base . '/schema', array(
      'methods'  => WP_REST_Server::READABLE,
      'callback' => array( $this, 'get_public_site_schema' ),
    ) );
  }
  /**
   * Get paths from all posts from the collection
   *
   * Get all posts without pagination, usefull when using getStaticPaths
   *
  ...
  public function get_paths(WP_REST_Request $request) {
   ...
   if ( empty( $posts ) ) {
     return new WP_Error( 'no-results', __( 'No posts found', 'text-domain' ), array( 'status' => 500 ) );
   }
   $names = array_map(function ($post) {
    ...
    if (!empty($names)) {
      ...
      return new WP_Error( 'no-results', __( 'Error making paths', 'text-domain' ), array( 'status' => 500 ) );
    }
  }
  /**
