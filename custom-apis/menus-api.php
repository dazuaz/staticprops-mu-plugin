...
add_action('rest_api_init', function () {
  ...
  register_rest_route($namespace, '/menus', [
    'methods' => 'GET',
    'callback' => 'staticprops_menus_get_all_menus',
  ]);

  register_rest_route($namespace, '/menus/(?P<id>[a-zA-Z0-9_-]+)', [
    'methods' => 'GET',
    'callback' => 'staticprops_menus_get_menu_data',
  ]);

  register_rest_route($namespace, '/locations/(?P<id>[a-zA-Z0-9_-]+)', [
    'methods' => 'GET',
    'callback' => 'staticprops_locations_get_menu_data',
  ]);

  register_rest_route($namespace, '/locations', [
    'methods' => 'GET',
    'callback' => 'staticprops_menu_get_all_locations',
  ]);
});
 ...
 */
function staticprops_menus_get_all_menus()
{
...
function staticprops_menu_get_all_locations()
{
 ...
 */
function staticprops_locations_get_menu_data($data)
{
  ...
  if (($locations = get_nav_menu_locations()) && isset($locations[$data['id']])) {
    ...
    $menu->items = staticprops_menus_get_menu_items($locations[$data['id']]);
  } else {
    return new WP_Error(
      'not_found',
 ...
 */
function staticprops_menus_get_menu_items($id)
{
  ...
  foreach ($menu_items as $key => $item) {
    ...
    if (isset($item->thumbnail_id) && $item->thumbnail_id) {
      $item->thumbnail_src = wp_get_attachment_image_url(
        intval($item->thumbnail_id),
        'post-thumbnail'
      );
    }
    if (isset($item->thumbnail_hover_id) && $item->thumbnail_hover_id) {
      $item->thumbnail_hover_src = wp_get_attachment_image_url(
        intval($item->thumbnail_hover_id),
        'post-thumbnail'
      );
    }
  ...
  do {
    foreach ($child_items as $key => $child_item) {
      if (staticprops_menus_dna_test($menu_items, $child_item)) {
...
function staticprops_menus_get_menu_data($data)
{
  ...
  if (has_nav_menu($data['id'])) {
    ...
    $menu = get_term($id);
    $menu->items = staticprops_menus_get_menu_items($id);
  } else {
    return new WP_Error(
      'not_found',
      'No menu has been found with this id or slug: `' .
        $data['id'] .
        '`. Please ensure you passed an existing menu ID, menu slug, location ID or location slug.',
      ['status' => 404]
    );
  }
